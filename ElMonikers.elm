...
dummyPlayers =
  [ newPlayer "Team Anna", newPlayer "Team Bob" ]
...
statsView players =
  case players of
    p :: _ ->
      div [ class "player" ] [ h2 [ class "name" ] [ text (p.name ++ " - " ++ toString p.score) ] ]
...
startView address =
  div [] [ button [ class "start", onClick address Init ] [ text "Start new game" ] ]
...
switchView address model =
  div
    [ id "switch" ]
    [ button
        [ class "next", onClick address Unpause ]
...
ticks : Signal Action
ticks =
  Signal.map (\_ -> Tick) (every second)
...
model : Signal Model
model =
  foldp update initialModel actions
...
update action model =
  case action of
    ...
    Later ->
      { model | cards = nextElem model.cards }
...
main : Signal Html
main =
  Signal.map (view actionsMB.address) model
